...
async function init() {
    ...
    document.getElementById('election-data-box').innerHTML = `
        <div><strong>${t.last}:</strong> ${t.parl} 15.10.2023 (${getDiff(now, new Date('2023-10-15'))} ${t.days}) | ${t.local} 07.04.2024 (${getDiff(now, new Date('2024-04-07'))} ${t.days})</div>
        <div><strong>${t.next}:</strong> ${t.parl} X 2027 (${t.about} ${getDiff(new Date('2027-10-17'), now)} ${t.days}) | ${t.local} IV 2029 (${t.about} ${getDiff(new Date('2029-04-08'), now)} ${t.days})</div>
    `;
